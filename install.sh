...
# --- venv + deps (reuse the existing venv on update; pip is a no-op when the
# pinned deps are already satisfied) ---
[ -d "$DEST/.venv" ] || { echo "Building venv ..."; python3 -m venv "$DEST/.venv"; }
"$DEST/.venv/bin/pip" install --quiet --upgrade pip
"$DEST/.venv/bin/pip" install --quiet -r "$DEST/requirements.txt"

# --- .env: preserved in place on update (rsync-excluded); generate on fresh ---
if [ "$UPDATE" -eq 0 ]; then
  cat > "$DEST/.env" <<ENV
